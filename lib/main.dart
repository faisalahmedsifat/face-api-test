            ...
            TextButton(
                ...
                onPressed: () {
                  ImagePicker().getImage(source: ImageSource.gallery).then(
                      (value) => setImage(
                          first,
                          io.File(value?.path as String).readAsBytesSync(),
    ...
    Regula.FaceSDK.matchFaces(jsonEncode(request)).then((value) {
      ...
      Regula.FaceSDK.matchFacesSimilarityThresholdSplit(
        ...
        var split = Regula.MatchFacesSimilarityThresholdSplit.fromJson(
            json.decode(str));
        setState(() => _similarity = (split?.matchedFaces.length as int) > 0
            ? (((split?.matchedFaces[0]?.similarity as double) * 100).toStringAsFixed(2) +
                "%")
            : "error");
      });
    });
